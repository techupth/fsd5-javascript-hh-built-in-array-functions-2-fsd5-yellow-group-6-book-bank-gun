...
function getTotalPrice(bill) {
  let salesData = bill.filter((info) => info.member !== null);
  let allPrice = salesData.reduce((accumulator, currentValue) => {
    accumulator[currentValue.location] =
      (accumulator[currentValue.location] || 0) + currentValue.total;
    return accumulator;
  }, {});
  return allPrice;

  //   let myArray =  bill.filter((info)=> info.member !== null)
  //   let member = myArray.map((allMember)=> allMember.location)
  //   let notSame = new Set([member])
  //   let total = myArray.reduce((acumulate,currentvalue)=>{
  //     console.log(acumulate)
  //     return acumulate + currentvalue.total},0)
  // console.log(total);
  // return notSame
}

const totalPrice = getTotalPrice(bills);
