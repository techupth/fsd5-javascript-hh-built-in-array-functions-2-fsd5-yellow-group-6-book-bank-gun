...
let totalPaidByLocaltion = bills.reduce((accumulator, bill) => {
  let location = bill.location;
  let total = bill.total;
  // Check if the location exists in the accumulator
  if (accumulator[location]) {
    // If yes, add the total to the existing sum
    accumulator[location] = accumulator[location] + total;
  } else {
    // If no, create a new entry for the location with the total
    accumulator[location] = total;
  }
  return accumulator;
}, {});

console.table(totalPaidByLocaltion);
